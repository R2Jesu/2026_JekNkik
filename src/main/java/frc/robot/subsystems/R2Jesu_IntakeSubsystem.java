...
public class R2Jesu_IntakeSubsystem extends SubsystemBase {
  ...
  private final SparkClosedLoopController intake_wheelController =
      intake_wheelsMotor.getClosedLoopController();
  ...
  public R2Jesu_IntakeSubsystem() {
    ...
    intake_wheelsMotor.configure(
        wheelsConfig, ResetMode.kResetSafeParameters, PersistMode.kNoPersistParameters);
  ...
  /**
   * Set intake wheel target velocity (RPM) using the closed-loop controller. This uses the
   * SparkClosedLoopController.setReference API. Confirm ControlType and ArbFFUnits names if your
   * REV library version differs.
   */
  ...
  public void periodic() {
    ...
    down_pidOutput =
        m_armDownController.calculate(
            intake_armEncoder.getDistance(), intake_armPositions[targetPosition]);
    up_pidOutput =
        m_armUpController.calculate(
            intake_armEncoder.getDistance(), intake_armPositions[targetPosition]);
    rpm =
        intakeRpmEntry != null
            ? intakeRpmEntry.getDouble(intake_lower_wheel_rpm)
            : intake_lower_wheel_rpm;
